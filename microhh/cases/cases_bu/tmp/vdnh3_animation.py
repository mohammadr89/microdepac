...
# Open datasets
ds_lai = xr.open_dataset(folder+'vdnh3_veg.xy.nc', decode_times=False)
ds_no_lai = xr.open_dataset(folder+'vdnh3_veg.xy_no_lai.nc', decode_times=False)

# For vdnh3_veg files, calculate global min/max like this:
vmin_global = min(
    ds_lai.vdnh3_veg.min(),
    ds_no_lai.vdnh3_veg.min()
)
vmax_global = max(
    ds_lai.vdnh3_veg.max(),
    ds_no_lai.vdnh3_veg.max()
)
...
ax[0].set_title("With LAI: Deposition Velocity at z=50m")
ax[1].set_title("No LAI: Deposition Velocity at z=50m")
