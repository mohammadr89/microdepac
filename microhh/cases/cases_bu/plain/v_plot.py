...
u = ds.groups['default'].variables['v'][:]  # u velocity
...
ax.set_xlabel('v [m/s]')
ax.set_ylabel('z [m]')
ax.set_title('V Velocgity Profile')
...
# Save the plot
plt.savefig('v_profile_timesteps.png', dpi=300, bbox_inches='tight')

# Print some basic statistics
print(f"Time range: {time[0]:.1f} to {time[-1]:.1f} seconds")
print(f"Height range: {z.min():.2f} to {z.max():.2f} m")
print(f"V velocgity range: {u.min():.2f} to {u.max():.2f} m/s")
