...
thl = ds.groups['thermo'].variables['thl'][:]  # liquid water potential temperature
...
for t in range(num_timesteps):
    ax.plot(thl[t, :], z, '-', color=colors[t], linewidth=1, alpha=0.7)

# Customize the plot
ax.set_xlabel('thl [K]')
ax.set_ylabel('z [m]')
ax.set_title('Liquid Water Potential Temperature Profile')
ax.grid(True, linestyle='--', alpha=0.5)

# Set axis limits similar to your example
ax.set_ylim(0, 4000)
ax.set_xlim(290, 310)
...
# Save the plot
plt.savefig('thl_profile_timesteps.png', dpi=300, bbox_inches='tight')

# Print some basic statistics
print(f"Time range: {time[0]:.1f} to {time[-1]:.1f} seconds")
print(f"Height range: {z.min():.2f} to {z.max():.2f} m")
print(f"Temperature range: {thl.min():.2f} to {thl.max():.2f} K")
