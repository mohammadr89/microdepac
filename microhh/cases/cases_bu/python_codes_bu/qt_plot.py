...
qt = ds.groups['thermo'].variables['qt'][:]  # total water mixing ratio
...
for t in range(num_timesteps):
    ax.plot(qt[t, :], z, '-', color=colors[t], linewidth=1, alpha=0.7)

# Customize the plot
ax.set_xlabel('qt [kg/kg]')
ax.set_ylabel('z [m]')
ax.set_title('Total Water Mixing Ratio Profile')
ax.grid(True, linestyle='--', alpha=0.5)

# Set axis limits
ax.set_ylim(0, 4000)
ax.set_xlim(0, 0.006)  # Based on your example plot
...
# Save the plot
plt.savefig('qt_profile_timesteps.png', dpi=300, bbox_inches='tight')

# Print some basic statistics
print(f"Time range: {time[0]:.1f} to {time[-1]:.1f} seconds")
print(f"Height range: {z.min():.2f} to {z.max():.2f} m")
print(f"Qt range: {qt.min():.6f} to {qt.max():.6f} kg/kg")
