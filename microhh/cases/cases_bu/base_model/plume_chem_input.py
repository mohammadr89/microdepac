...
thl = profile(zi=1000, v_bulk=295,   dv=4,     gamma_v=0.008)  # Warmer, more stable
#qt  = profile(zi=1000, v_bulk=10e-3, dv=-2e-3, gamma_v=-0.003e-3, clip_at_zero=True)
#qt  = profile(zi=1000, v_bulk=6e-3, dv=-2e-3, gamma_v=-0.002e-3, clip_at_zero=True) ##maarten's version
qt  = profile(zi=1500, v_bulk=5e-3, dv=-4e-3, gamma_v=-0.005e-3, clip_at_zero=True) # Drier
u   = np.ones(ktot) * 5
co2 = np.zeros(ktot)
...
wthl = 0.25 * np.sin(np.pi * (time-t0) / td) # Increased sensible heat flux
#wthl = 0.2 * np.sin(np.pi * (time-t0) / td) #maarten's version
#wqt  = 8e-5 * np.sin(np.pi * (time-t0) / td)
wqt  = 2e-5 * np.sin(np.pi * (time-t0) / td) #reduced moisture flux
...
if (sw_land_surface):
    ...
    set_value('c_veg', forest=0.0, grass=0.0)      # Vegetation fraction
    set_value('lai', forest=0.0, grass=0.0)        # Leaf Area Index
    ...
    # Surface parameters
    ls['gD'][:,:] = 0.06               # Higher VPD sensitivity
    ls['rs_veg_min'][:,:] = 300        # Higher vegetation resistance
    ls['rs_soil_min'][:,:] = 200       # Higher soil resistance
    ls['lambda_stable'][:,:] = 20      # Higher heat conductivity
    ls['lambda_unstable'][:,:] = 20    # Higher heat conductivity
    ls['cs_veg'][:,:] = 0.5            # Moderate vegetation heat capacity
    ls['t_bot_water'][:,:] = 298       # Warmer water temperature
                                                                           
    # Soil properties (4 layers)       
    ls['t_soil'][:,:,:] = 295          # Warmer soil
    ls['theta_soil'][:,:,:] = 0.1      # Drier soil
    ls['index_soil'][:,:,:] = 0        # Sandy soil (less water retention)
    ls['root_frac'][:,:,:] = 0.15      # Less root fraction


 
    ## Surface parameters
    #ls['gD'][:,:] = 0.06              # Vegetation water stress parameter
