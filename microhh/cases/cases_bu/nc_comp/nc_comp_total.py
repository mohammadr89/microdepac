...
depac_file = 'vdnh3.xy_depac.nc'
ifs_file = 'vdnh3.xy_ifs.nc'
...
difference = ds_depac['vdnh3'] - ds_ifs['vdnh3']
...
def plot_difference_at_time(difference, time_index):
    ...
    plt.colorbar(im, label='Total NH3 deposition velocity difference (DEPAC - IFS)')
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    plt.title(f'Total NH3 deposition velocity difference at time {time_value} s')
...
difference.to_netcdf('vdnh3_total_difference.nc')
