...
nh3 = ds.groups['default'].variables['nh3'][:]  # NH3 concentration
...
for t in range(num_timesteps):
    ax.plot(nh3[t, :], z, '-', color=colors[t], linewidth=1, alpha=0.7)

# Customize the plot
ax.set_xlabel('nh3 [mol/mol]')
ax.set_ylabel('z [m]')
ax.set_title('NH3 Profile')
...
min_nh3 = np.min(nh3)
max_nh3 = np.max(nh3)
ax.set_xlim(min_nh3 - 0.1*(max_nh3-min_nh3), max_nh3 + 0.1*(max_nh3-min_nh3))
...
# Save the plot
plt.savefig('nh3_profile_timesteps.png', dpi=300, bbox_inches='tight')

# Print some basic statistics
print(f"Time range: {time[0]:.1f} to {time[-1]:.1f} seconds")
print(f"Height range: {z.min():.2f} to {z.max():.2f} m")
print(f"NH3 range: {nh3.min():.2e} to {nh3.max():.2e} mol/mol")
