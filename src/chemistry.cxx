 ...
 * z_target     = float   : Target height for concentration that is used in flux calculations [m]
 ...
 * sw_use_cstar       = boolean : Use cstar for flux calculation (default: true)
 * 
 ...
 * c_diff             : Concentration difference [ppb]
            ...
            #pragma ivdep
                for (int i=istart; i<iend; ++i)
                {
                    ...
                    if (k==kstart)
                    {
                        ...
                        {
                            z_target = z_fixed;  // Use constant height
                        }
                        else
                        {
                            z_target = rsl_ratio * z0m[ij];  // Use adaptive height
                        }
                        ...
                        else
                        {
                            ...
                            {
                                cstar1[ij] = TF(0.0);  // Added this else clause for safety
                            }
                            
                            // Calculate extrapolated concentration at z_target using cstar1
                            const TF scaling_factor = calc_factor(z_1, z_target, L);
                            c_target[ij] = c_1 + cstar1[ij] * scaling_factor;
                            
                            // Find closest grid point to z_target
                            int k_closest = kstart;
                            TF min_distance = std::abs(z[kstart] - z_target);
                            
                            for (int k_grid = kstart; k_grid < kend; ++k_grid)
                            {
                                TF distance = std::abs(z[k_grid] - z_target);
                            ...
                            const int ijk_closest = i + j*jstride + k_closest*kstride;
                            c_grid_closest[ij] = nh3[ijk_closest];
                            
                            // Calculate concentration difference in ppb 
                            const TF concentration_diff = (c_target[ij] - c_grid_closest[ij]) * TF(1e9);
                            c_diff[ij] = concentration_diff;
                            
                            // ========================================
                            // Now calculate flux_inst based on sw_use_cstar
                            // ========================================
                            TF concentration_for_flux;
                            
                            if (sw_use_cstar)
                            {
                                // Use extrapolated concentration (cstar method)
                                concentration_for_flux = c_target[ij];
                            }
                            else
                            {
                                // Use closest grid concentration
                                concentration_for_flux = c_grid_closest[ij];
                            }
                            
                            // Calculate flux
                            flux_inst[ij] = (-1.0) * vdnh3[ij] * concentration_for_flux * rhoref[k] * xmair_i * xmnh3;
                            
                            // Calculate decay with concentration scaling (DIFFERENT from step 2!)
                            if (std::abs(nh3[ijk]) > TF(1e-15))
                            {
                                decay = (vdnh3[ij] * dzi[k] * concentration_for_flux / nh3[ijk]) + lti;
                            }
                            else
                            {
                                decay = lti;
    ...
    , field3d_operators(master, grid, fields)  // ← Step 5: Uses members (already set!) // added Radiation<TF>& radiationin parameter and member variable
{
    ...
    sw_use_cstar = inputin.get_item<bool>("chemistry", "sw_use_cstar", "", true);
    ...
    master.print_message("Use cstar for flux calculation = %s \n", sw_use_cstar ? "true" : "false");
        ...
        Stats<TF>& stats, Cross<TF>& cross)
{
    ...
    if (stats.get_switch())
    {
        ...
        stats.add_time_series("c_diff", "Concentration difference (c_target - c_grid_closest)", "ppb", group_named);
    ...
    if (cross.get_switch())
    {
        ...
        std::vector<std::string> allowed_crossvars = {"vdnh3","flux_nh3","flux_inst","total_flux_mol_ha","cstar1","cstar2","c_grid_closest","c_target","c_diff"};
